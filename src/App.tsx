...
export default function App() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setState('done');
    } catch (e: any) { setState('idle'); alert("שגיאה במערכת: " + e.message); }
  };
    ...
    <div className="max-w-[420px] mx-auto min-h-screen bg-bg-light dark:bg-bg-dark text-text-light dark:text-text-dark font-heebo p-4" dir="rtl">
      ...
      {state === 'idle' ? (
        <div className="space-y-6">
          ...
          <button onClick={generate} className="w-full py-4 bg-primary text-white rounded-xl font-bold shadow-lg">צור מתכון</button>
        </div>
      ) : state === 'busy' ? (
